# EXACT — 감정평가 의견서 자동화 도구

**EXACT** = **EX**pert **A**ppraisal **C**ompanion **T**ool (구 명칭: ARAP)

집합건물 감정평가 의견서 작성의 단순작업(시점수정 계산, 요인비교표, 표준문구, DOCX 생성)을
자동화하는 개인 제작 도구입니다.

- 실행(웹): https://wjjung111.github.io/woojin/
- 실행(로컬): `EXACT 실행.bat`

## ⚠️ 저작권 안내 / Copyright Notice

**Copyright (c) 2026 정우진 감정평가사 (Woojin Jung, Certified Appraiser). All Rights Reserved. (모든 권리 보유)**

본 저장소는 열람 및 앱 실행 목적으로만 공개되어 있습니다.
**저작권자의 사전 서면 동의 없는 소스코드의 복제·수정·상업적 이용·재배포·2차적 저작물 작성을 금지합니다.**
법인·사무소 내부 업무용 개조 및 배포도 상업적 이용에 포함됩니다.

This repository is public for viewing and running the app only.
**Copying, modifying, commercial use, redistribution, or creating derivative works
without prior written permission is prohibited** — including internal adaptation
within an appraisal firm. See [LICENSE](LICENSE) for details.

이용 허락 문의: wjjung9334@gmail.com
